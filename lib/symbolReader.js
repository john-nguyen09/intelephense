    ...
    preorder(node, spine) {
        ...
        switch (node.phraseType) {
            ...
            case 121 /* NamespaceName */:
                ...
                break;
            case 91 /* GlobalDeclaration */:
                this._transformStack.push(new GlobalVariableTransform(this.nameResolver, this.document.nodeHashedLocation(node), this.lastPhpDoc, this.lastPhpDocLocation));
                break;
            case undefined:
                //tokens
    ...
    function assignPhpDocInfoToSymbol(s, doc, docLocation, nameResolver) {
        ...
        switch (s.kind) {
            ...
            case 2 /* Interface */:
                ...
                break;
            case 8192 /* GlobalVariable */:
                tag = doc.findGlobalTag(s.name);
                if (tag) {
                    s.doc = symbol_1.PhpSymbolDoc.create(doc.text, typeString_1.TypeString.nameResolve(tag.typeString, nameResolver));
                }
                break;
            default:
                break;
...
class GlobalVariableTransform {
    constructor(nameResolver, location, doc, docLocation) {
        this.nameResolver = nameResolver;
        this.doc = doc;
        this.docLocation = docLocation;
        this.symbol = symbol_1.PhpSymbol.create(8192 /* GlobalVariable */, '', location);
        this.symbol.children = [];
        this.symbol.associated = [];
    }
    push(transform) {
        if (transform instanceof SimpleVariableTransform) {
            this.symbol.name = transform.symbol.name;
