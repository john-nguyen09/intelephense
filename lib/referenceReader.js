...
class ReferenceReader {
    constructor(doc, nameResolver, symbolStore) {
        ...
        let globalVariables = this.symbolStore.filter((symbol) => {
            return symbol.kind === 8192 /* GlobalVariable */;
        });
        for (let globalVariable of globalVariables) {
            if (!globalVariable.type && globalVariable.doc) {
                globalVariable.type = typeString_1.TypeString.nameResolve(globalVariable.doc.type, this.nameResolver);
            }
        ...
        switch (node.phraseType) {
            ...
            case 156 /* SimpleVariable */:
                ...
                for (let i = spine.length - 1; i >= 0; i--) {
                    if (spine[i].phraseType == 91 /* GlobalDeclaration */) {
                        isGlobal = true;
                        break;
                    }
                }
    ...
    _methodDeclaration(node) {
        ...
        if (type && symbol) {
            let fn = (x) => {
                return x.kind === 32 /* Method */ && symbol.name === x.name;
            };
...
class GlobalVariableTransform {
    constructor(loc, varTable) {
        ...
        this.phraseType = 156 /* SimpleVariable */;
        this.reference = reference_1.Reference.create(8192 /* GlobalVariable */, '', loc);
    }
    push(transform) {
        if (transform.tokenType === 84 /* VariableName */) {
            this.reference.name = transform.text;
