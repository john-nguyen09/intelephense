...
class AbstractNameCompletion {
    ...
    _toCompletionItem(s, useDeclarationHelper, namespaceName, isUnqualified, fqnOffset) {
        const item = {
            kind: symbolKindToLspSymbolKind(s.kind),
            label: isUnqualified ? symbol_1.PhpSymbol.notFqn(s.name) : s.name.slice(fqnOffset)
        };
        ...
        if (isUnqualified && s.kind === 512 /* Namespace */) {
            item.label = s.name;
        ...
        if (!isUnqualified) {
            ...
            labelExtra = `[use ${s.associated[0].name}]`;
            item.insertText = item.label;
        }
        else if ((!namespaceName && !symbolNamespace) || (s.kind === 8 /* Constant */ && this._isMagicConstant(s.name))) {
            ...
            labelExtra = `[namespace ${namespaceName}]`;
            item.insertText = item.label;
        }
        else if (namespaceName && !symbolNamespace) {
        ...
        if (s.kind === 64 /* Function */) {
            //add a short representation of the signature
            //and configure to invoke parameter hints
            item.label += symbol_1.PhpSymbol.signatureString(s, true);
...
class InstanceOfTypeDesignatorCompletion extends AbstractNameCompletion {
    ...
    _symbolFilter(s) {
        return (s.kind & (1 /* Class */ | 2 /* Interface */)) > 0 && !(s.modifiers & (512 /* Anonymous */));
    }
...
class ClassTypeDesignatorCompletion extends AbstractNameCompletion {
    ...
    _toCompletionItem(s, useDeclarationHelper, namespaceName, isUnqualified, fqnOffset) {
        ...
        if (constructor && symbol_1.PhpSymbol.hasParameters(constructor)) {
            ...
            item.label += symbol_1.PhpSymbol.signatureString(constructor, true) + labelExtra;
            item.insertText += '($0)';
            item.insertTextFormat = lsp.InsertTextFormat.Snippet;
            item.command = triggerParameterHintsCommand;
        }
...
class MemberAccessCompletion {
    ...
    toMethodCompletionItem(s) {
        let item = {
            ...
            label: s.name + symbol_1.PhpSymbol.signatureString(s, true)
        };
        if (s.doc && s.doc.description) {
            item.documentation = s.doc.description;
        }
