...
class MemberAccessCompletion {
    ...
    toPropertyCompletionItem(s) {
        let item = {
            ...
            label: (s.modifiers & 32 /* Static */) > 0 ? s.name : s.name.slice(1),
...
class NamespaceUseClauseCompletion {
    ...
    canSuggest(traverser) {
        ...
        return parsedDocument_1.ParsedDocument.isPhrase(traverser.parent(), [121 /* NamespaceName */]) &&
            parsedDocument_1.ParsedDocument.isPhrase(traverser.parent(), [124 /* NamespaceUseDeclaration */, 122 /* NamespaceUseClause */]);
    }
    completions(traverser, word) {
        let items = [];
...
class NamespaceUseGroupClauseCompletion {
    ...
    canSuggest(traverser) {
        ...
        return parsedDocument_1.ParsedDocument.isPhrase(traverser.parent(), [121 /* NamespaceName */]) &&
            parsedDocument_1.ParsedDocument.isPhrase(traverser.parent(), [125 /* NamespaceUseGroupClause */]);
    }
    completions(traverser, word) {
        let items = [];
        if (!word) {
...
class MethodDeclarationHeaderCompletion {
    ...
    _magicMethodCompletionItems(word, excludeMap) {
        let name;
        const items = [];
